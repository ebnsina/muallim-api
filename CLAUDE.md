...
**One spelling.** British throughout — enrol, enrolment, organise, catalogue, authorisation. `Authorization` is an HTTP header, `catalog` and `enroll` are Go packages, and CSS has `color`: those are code, and code is not prose.
