...
**The dependency rule.** `platform` imports nothing from the project. Domain packages (`tenant`, `auth`, `catalog`, `assess`, `enroll`, `commerce`, `media`, `learn`, `comms`, plus the institution domains `academics`, `exams`, `fees`, `staff`, `notices`, `hifz`, `overview`, the operations domains `library`, `transport`, `hostel`, `payroll`, `ledger`, `calendar`, `admissions`, `idcard`, and the two standalone builders `certdesign` (certificate canvas) and `coursebuild` (course blueprint)) may import `platform` but never `internal/httpapi`, and never each other — cross-domain needs go through an interface the *caller* defines, wired in `cmd/`. `internal/httpapi` imports domains; nothing imports it.
