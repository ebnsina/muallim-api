...
- **The interface is English.** Bengali and Arabic (RTL) were the plan and are not built: there is no translation layer and no `dir="rtl"` anywhere, by a deliberate decision to finish the product first. Say so rather than implying otherwise — the landing page sold "Arabic right-to-left curricula" for months on the strength of this line. A school writes its own courses in whatever language it teaches in, which is a different thing and true.
- **Institution types are BD-weighted.** Madrasa and coaching are first-class alongside school/college. Report-card grading ships **one** scale — GPA 5.0 (Bangladesh), banded A+ to F — and lets a workspace band its own for anything else. The madrasa ladder is *class names a school types in* (`grade_levels`, named and ranked by whoever runs the place), not a scale we ship: saying otherwise in a doc was how the landing page came to promise a feature that did not exist.
...
The product is growing from an LMS into an institution super-app (school/college/madrasa/coaching + attendance, exams/report cards, fees, timetable, staff, guardian notices, and the operations layer — library, transport, hostel, payroll, an income/expense ledger, and an academic calendar). A **parent-and-pupil portal** (`portal:read`, `RoleGuardian`) lets a guardian or student read their own child's attendance, fees, and hifz — gated twice, by permission and by an ownership check (`academics.ChildStudent`) so one family never reads another's. **Admissions** takes applications and, on admit, creates the student and guardian in one HTTP-layer orchestration across `admissions` + `academics`. Two **standalone visual builders** — a certificate designer (`certdesign`) and a course-blueprint builder (`coursebuild`) — plus an **ID-card designer** (`idcard`) store free-positioned canvas layouts (a jsonb layout + an object-store background), rendered client-side. The **academic spine** is `academics`: the institution's type (school/college/madrasa/coaching), its academic years and terms with one marked current, and its classes, sections and subjects — the rows attendance, exams, fees, hifz, the timetable, admissions and reports all read. A workspace writes its own mail with `automation`. The academic layer is new domains under the existing contract; see `docs/plan.md`.
...
**The dependency rule.** `platform` imports nothing from the project. Domain packages (`tenant`, `auth`, `catalog`, `assess`, `enroll`, `commerce`, `media`, `learn`, `comms`, plus the institution domains `academics`, `exams`, `fees`, `staff`, `notices`, `hifz`, `overview`, the operations domains `library`, `transport`, `hostel`, `payroll`, `ledger`, `calendar`, `admissions`, `idcard`, `liveclass` (bring-your-own-link live sessions), the catalogue domains `taxonomy` (course categories + tags), `bundle` (grouped courses), `learnpath` (ordered course track), the real-time `chat` (course channels, DMs, groups — REST plus a WebSocket over Postgres LISTEN/NOTIFY, no Redis), `automation` (a workspace's own rules for the mail it sends), and the two standalone builders `certdesign` (certificate canvas) and `coursebuild` (course blueprint), plus `leads` (demo requests, the one domain with no tenant)) may import `platform` but never `internal/httpapi`, and never each other — cross-domain needs go through an interface the *caller* defines, wired in `cmd/`. `internal/httpapi` imports domains; nothing imports it.
...
**A demo request is the exception that proves the tenant rule.** Somebody asking to be shown the product has no workspace — that is the whole content of the request — so `demo_requests` is the only table besides `tenants` with no `tenant_id` and no RLS policy, written through `WithoutTenant`, on a path exempt from tenant resolution (`systemPaths`) because it arrives on a host that resolves to no tenant. Read it as an exception, never as a template: the same shape in a domain that *does* have a workspace is a cross-tenant leak. It is unauthenticated, so it is rate-limited, it echoes nothing back, and it answers every well-formed request identically — a response that varied by what we already hold would tell a stranger with a list whether an address is in our database. See `docs/modules/demo-requests.md`.
...
**A workspace writes its own mail; the code only decides when.** Four messages were hard-coded — verification, reset, invitation, digest — so a school that wanted to welcome the people who enrol had to ask somebody to remember. `automation` is a rule: an event says when, a subject and body say what, and `{{placeholders}}` are filled from what happened. The event set is closed (a `CHECK`, and a Go catalogue), because a rule for an event nothing fires never runs and its author has no way to find that out; and a template naming a placeholder its event cannot fill is refused while the author is looking at it, not discovered by a learner reading a gap. Rules fire in the transaction that recorded the event, and never fail it — a learner turned away from a course because a welcome note misfired is the worse outcome, so the adapter logs and swallows. Enrolling twice welcomes nobody twice: the fire sits below the `created` guard.

**A role the code knows must be nameable on the wire.** `RoleGuardian` had a permission, a widened role check in the database, and an ownership gate written for it — and every role `enum` on the wire listed the four that predated it, so no invitation could name a guardian and the parent-and-pupil portal shipped as a feature nobody could sign in to. The lesson is not "remember the enum": it is that a role, a status, or a kind added anywhere is added *everywhere it is named*, and that a feature nothing links to and nobody can reach is not shipped, whatever the tests say. A link is part of the feature: `/portal` existed for weeks with nothing anywhere pointing at it.

**The screens speak the reader's language; the code keeps the domain's.** A teacher met "Course taxonomy", "Blueprint name", "set ANTHROPIC_API_KEY", "Sslcommerz" and "restricted" — our words, leaking through the glass. So the screens say Organise courses, Course plan, "AI isn't set up yet — ask your administrator", SSLCommerz, and "Needs more details before it can take payments", while `taxonomy`, `blueprint`, `catalog` and `enroll` stay exactly what they are in Go: the two repos disagreeing about a domain's name costs more than it buys. The seam is the wire. And an API message is user-visible text — muallim-web renders `detail` verbatim, so "author must be a uuid" was a sentence we showed a teacher. Write them for the person who reads them, and never `err.Error()`.

**One spelling: American.** enroll, enrollment, organize, catalog, authorization, center. This file said "British throughout" while `muallim-web/CLAUDE.md` said "the copy is American" — two repos, one product, opposite rules, and neither mentioned the other. A convention that contradicts its sibling is worse than no convention: it makes every reviewer right, whichever way they argue.

**The wire is not prose, and it does not move.** `enrolments` is already a table, `/v1/me/enrolments` and `/v1/courses/{slug}/enrolments` are already paths, and `cancel-enrolment`, `grant-enrolment`, `import-enrolments`, `list-my-enrolments` are already OperationIDs. Those stay spelt as they are, forever, because the spec is a public interface and renaming an OperationID breaks a client nobody can see — a spelling preference is not worth a breaking change. `catalog` and `enroll` are Go packages, `Authorization` is an HTTP header, CSS has `color`. Code is not prose, and the same word can be spelt two ways in one file when one of them is an identifier.

**The prose is not swept yet.** Roughly a thousand comments and docs across this repo still read British, and every new line should be American. This is deliberate debt, recorded rather than pretended away: a mechanical sweep touching a thousand comments is a diff nobody can review beside real work, and it can wait for a commit of its own.
