...
func TestAcademicsSentinelsMapToADeliberateStatus(t *testing.T) {
	...
	tests := map[error]int{
		...
		academics.ErrInvalidAttendance:      http.StatusUnprocessableEntity,
		academics.ErrInvalidPeriod:          http.StatusUnprocessableEntity,
		academics.ErrInvalidPromotion:       http.StatusUnprocessableEntity,
...
func TestExamsSentinelsMapToADeliberateStatus(t *testing.T) {
	t.Parallel()

	tests := map[error]int{
		exams.ErrNotFound:      http.StatusNotFound,
		exams.ErrExamPublished: http.StatusConflict,
		exams.ErrNoScale:       http.StatusUnprocessableEntity,
		exams.ErrUngraded:      http.StatusUnprocessableEntity,
		exams.ErrInvalidPage:   http.StatusUnprocessableEntity,
		exams.ErrInvalidScale:  http.StatusUnprocessableEntity,
		exams.ErrInvalidExam:   http.StatusUnprocessableEntity,
		exams.ErrInvalidMark:   http.StatusUnprocessableEntity,
	}

	for err, want := range tests {
		if got := statusOf(examsError(err)); got != want {
			t.Errorf("%v mapped to %d, want %d", err, got, want)
		}
...
func TestHifzSentinelsMapToADeliberateStatus(t *testing.T) {
	t.Parallel()

	tests := map[error]int{
		hifz.ErrNotFound:     http.StatusNotFound,
		hifz.ErrInvalidPage:  http.StatusUnprocessableEntity,
		hifz.ErrInvalidEntry: http.StatusUnprocessableEntity,
	}

	for err, want := range tests {
		if got := statusOf(hifzError(err)); got != want {
			t.Errorf("%v mapped to %d, want %d", err, got, want)
		}
		wrapped := fmt.Errorf("hifz: doing a thing: %w", err)
		if got := statusOf(hifzError(wrapped)); got != want {
			t.Errorf("wrapped %v mapped to %d, want %d", err, got, want)
		}
	}
}
...
func TestLearnSentinelsMapToADeliberateStatus(t *testing.T) {
	...
	tests := map[error]int{
		...
		learn.ErrQuestionNotFound:  http.StatusNotFound,
		learn.ErrAnswerNotFound:    http.StatusNotFound,
		learn.ErrEmptyPost:         http.StatusUnprocessableEntity,
		learn.ErrPostTooLong:       http.StatusUnprocessableEntity,
	}
